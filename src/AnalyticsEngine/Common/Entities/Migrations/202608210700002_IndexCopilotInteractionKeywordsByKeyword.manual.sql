/* =====================================================================================================
   ...
   Migration: 202608210700002_IndexCopilotInteractionKeywordsByKeyword
   =====================================================================================================
   ...
   WHAT IT DOES
     Adds an index leading on keyword_id to the Copilot keyword link table:
       * IX_copilot_interaction_keywords_keyword_id
             ON copilot_interaction_keywords (keyword_id, interaction_id)

     WHY (issue #296). The table ships with a single index leading on interaction_id, but both orphan-
     keyword cleanups probe it the other way round (ck.keyword_id = k.id), in "Clean Old Data Data.sql"
     and the per-user purge in "Clean Data By User StoredProc.sql". Without a seekable path SQL Server
     scans the whole link index once per candidate keyword.

     MEASURED (synthetic scale: 500,000 keywords, 5,000,000 link rows; medians of 6 runs):
       targeted per-user purge   before 11,256 reads / 155 ms  ->  after 137 reads / 16 ms  (Scan -> Seek)
       bulk retention sweep      before 11,256 reads / 797 ms  ->  after 11,281 reads / 437 ms
     The bulk sweep legitimately stays a scan (every keyword is a candidate); the per-user purge is where
     the seek pays - 82x fewer logical reads.
   SAFETY
   ...
   UPGRADE TIME
     * copilot_interaction_keywords is CREATED BY THIS SAME RELEASE, so on an upgrade from the previous
       stable build the table is EMPTY and this is instant.
     * It only takes real time on a database already running the Copilot interaction-history import from a
       dev build: synthetic 5,000,000 link rows built in 7.4 s occupying 87 MB (~1.5 s per million rows).
   PREREQUISITE
     The database must already be on migration 202608210700001_WidenCopilotAccessedResourceDedupIndex. The __MigrationHistory stamp
...
-- =====================================================================================================
IF NOT EXISTS (SELECT 1 FROM dbo.__MigrationHistory WHERE MigrationId = N'202608210700002_IndexCopilotInteractionKeywordsByKeyword')
BEGIN
    IF EXISTS (SELECT 1 FROM dbo.__MigrationHistory WHERE MigrationId = N'202608210700001_WidenCopilotAccessedResourceDedupIndex')
    BEGIN
        INSERT INTO dbo.__MigrationHistory (MigrationId, ContextKey, Model, ProductVersion)
        SELECT N'202608210700002_IndexCopilotInteractionKeywordsByKeyword', ContextKey, Model, ProductVersion
        FROM dbo.__MigrationHistory
        WHERE MigrationId = N'202608210700001_WidenCopilotAccessedResourceDedupIndex';
        RAISERROR('IndexCopilotInteractionKeywordsByKeyword: recorded in __MigrationHistory.', 0, 1) WITH NOWAIT;
    END
    ELSE
        RAISERROR('IndexCopilotInteractionKeywordsByKeyword: the schema change was applied, but prerequisite migration 202608210700001_WidenCopilotAccessedResourceDedupIndex is missing from __MigrationHistory, so it was NOT stamped. Apply the earlier migrations first, or run the installer to reconcile.', 16, 1);
END
ELSE
    RAISERROR('IndexCopilotInteractionKeywordsByKeyword: already recorded in __MigrationHistory, nothing to do.', 0, 1) WITH NOWAIT;

namespace Common.Entities.Migrations
{
    ...
    /// <summary>
    /// De-duplicates <c>dbo.urls.full_url</c>, re-points every known dependent row to the
    /// canonical URL, and rebuilds <c>IX_urls_full_url</c> as a unique index.
    ///
    /// The migration is guarded, set-based and resumable. It materialises each dependent work-set
    /// once, then applies updates/deletes in small batches outside the EF transaction. Before a
    /// duplicate URL is deleted, every known reference is checked again so cascade-delete FKs
    /// cannot silently remove customer activity. Run large upgrades in a maintenance window with
    /// the importers stopped.
    ///
    /// <c>IGNORE_DUP_KEY</c> keeps concurrent check-then-insert lookup races from aborting an
    /// importer batch after uniqueness is enforced; SQL Server skips only the duplicate URL row.
    /// </summary>
    public partial class EnforceUniqueUrlsFullUrl : DbMigration
    {
...
IF OBJECT_ID(N'dbo.urls', N'U') IS NOT NULL
   AND EXISTS (
       ...
       WHERE object_id = OBJECT_ID(N'dbo.urls')
         AND name = N'IX_urls_full_url')
BEGIN
    CREATE NONCLUSTERED INDEX [IX_urls_full_url]
    ON [dbo].[urls] ([full_url])
...
";
        ...
        {
            Console.WriteLine("DB SCHEMA: Applying 'EnforceUniqueUrlsFullUrl'. De-duplicates dbo.urls, re-points dependent activity, and makes IX_urls_full_url unique. This can update large fact tables and rebuild an index; stop importers and use a maintenance window.");
            Sql(Up_Sql, suppressTransaction: true);
        }

        public override void Down()
        {
            Console.WriteLine("DB SCHEMA: Reverting 'EnforceUniqueUrlsFullUrl' to a non-unique index. De-duplicated URL rows cannot be restored.");

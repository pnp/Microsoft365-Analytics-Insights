namespace Common.Entities.Migrations
{
    ...
    ///
    /// <para>
    /// <b>Measured before / after</b> - the <c>LicensedUsers</c> query as actually generated by
    /// <see cref="Common.Entities.CopilotAdoption.CopilotAdoptionSql"/> (extracted from the compiled
    /// assembly, not a hand-written approximation), medians of 3 warm runs with the cold run discarded,
    /// <c>DBCC FREEPROCCACHE</c> before each run and <c>OPTION (RECOMPILE)</c> in the query, on a synthetic
    /// bench of 200,000 users / 60,000 Copilot seats / 12.5M <c>audit_events</c> / 12M <c>copilot_chats</c>
    /// carrying the production index set:
    /// </para>
    /// <code>
    ///   window | before (join, pre-migration) | after (denormalised) | logical reads
    ///   -------+------------------------------+----------------------+-----------------------
    ///     28d  | 73,589 ms                    | 22,525 ms   (3.3x)   | 1,374,520 -> 742,124
    ///     90d  | 98,165 ms   TIMED OUT        | 41,399 ms   (2.4x)   | 1,387,552 -> 796,728
    /// </code>
    /// <para>
    /// Both elapsed time and logical reads improve, at both selectivities. At 90 days the old query exceeded
    /// the 90-second command timeout outright, which is the production failure reproduced.
    /// </para>
    /// <para>
    /// Measured on the SAME database state (i.e. the old query re-run after this migration, so both pay for
    /// the wider table) the gap is larger still - 91,010 ms -&gt; 22,525 ms at 28 days and 117,825 ms -&gt;
    /// 41,399 ms at 90 days. The table above deliberately quotes the smaller, honest figure: it compares
    /// what an administrator actually had before the upgrade with what they get after it.
    /// </para>
    ///
    /// <para>
    /// <b>Index shape.</b> <c>(time_stamp, user_id) INCLUDE (app_host, agent_id)</c>, keyed on
    /// <c>time_stamp</c> FIRST. That was measured, not assumed. Keying on <c>user_id</c> first is very slightly
    /// better for <c>LicensedUsers</c> alone (19,495 ms vs 20,333 ms at 28d on the prototype) because that one
    /// query reads the full 365-day history and so has no date selectivity to exploit - but every OTHER
    /// Copilot query is window-scoped, and there a leading <c>time_stamp</c> is twice as fast: the
    /// <c>UsageByApp</c> aggregate over a 28-day window measured 2,986 ms with <c>(time_stamp, user_id)</c>
    /// against 5,827 ms with <c>(user_id, time_stamp)</c>. The clustering key <c>event_id</c> rides along as
    /// the row locator, so queries that join the Copilot detail tables still get it without a lookup.
    /// </para>
    ///
    /// <para>

...
- **Removing a package or project reference can silently DOWNGRADE a transitive dependency**, and that breaks at runtime rather than at build time. If the removed reference was what pulled in the highest version of something (e.g. `Microsoft.IdentityModel.Protocols.OpenIdConnect` 8.x, `System.IO.Hashing` 10.x), NuGet falls back to an older version while the binding redirect in `*.Template.config` still points at the newer one, so the process throws `FileLoadException` the first time it loads that assembly. The build only says `MSB3277: Found conflicts between different versions`. Before shipping such a change, compare every assembly version in the new output against the previous release and pin anything that went down with a direct `PackageReference`.

## Where a dependency lives decides which release packages carry it
The release ships four .NET packages - `AppInsightsImporter.zip`, `Office365ActivityImporter.zip`, `Website.zip` and `ControlPanelApp.zip` - and **every one of them references `Common/DataUtils` and `Common/Entities`**. A `PackageReference` added to either of those, or to `Common/UsageReporting`, is copied into every package it reaches, whether or not that package ever calls it. That is how the Cosmos DB SDK (used only by `src/TelemetryService`), BouncyCastle (used only by the installer's `StringCipher`) and the Azure Resource Manager SDKs (via the web app's old reference to `App.ControlPanel.Engine`) each ended up in packages that never used them.
- Put a dependency in the lowest project that actually **calls** it: installer-only code and packages belong in `App.ControlPanel.Engine`, importer-only ones in the importer engine, and maintainer-side ones in `src/TelemetryService`.
- The web app must not reference `App.ControlPanel.Engine` or the importer engines. Shared contracts it needs (config JSON shapes, call-records rules and DTOs) belong in `Common.Entities`; code that needs a heavy SDK stays at the edge that owns it (the importer keeps the Graph SDK; the web app reads the calls webhook subscription over Graph REST).
- `.github/scripts/Build-ReleasePackage.ps1`, which `ci.yml` and `pr.yml` use to zip each package, fails the build when a known offender reappears in a package it must not ship in. If it fails, move the reference - don't relax the rule.

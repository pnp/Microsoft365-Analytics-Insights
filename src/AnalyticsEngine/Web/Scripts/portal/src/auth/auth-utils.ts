...
/** True when running inside an iframe (replaces the removed msal v1 WindowUtils.isInIframe). */
...
export const msalApp = new PublicClientApplication({
  ...
  cache: {
    cacheLocation: 'localStorage', // "sessionStorage" is more secure, but "localStorage" gives you SSO.
  },
  system: {
    loggerOptions: {
...
// MSAL v3+ requires an explicit, awaited initialize() before any other API is used.

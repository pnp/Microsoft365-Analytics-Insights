...
-- 2. Upsert client types (Teams / Mobile / Desktop / Web)
...
-- 3. Insert apps we haven't seen before (set first_seen_at to the earliest staged event)
INSERT INTO power_apps (app_id, [name], environment_id, first_seen_at)
SELECT d.app_id, d.app_name, env.id, d.first_seen_at
FROM (
    SELECT i.app_id,
...
-- 4. Backfill friendlier app metadata if it arrives later
UPDATE pa
...
WHERE (pa.[name] IS NULL OR pa.[name] = pa.app_id OR pa.environment_id IS NULL);
...
-- 6. Refresh connector bindings emitted on publish events.

...
namespace Tests.UnitTests
{
    ...
    public class CopilotExtendedDataTests
    {
        ...
        [TestMethod]
        public void SerializeAgentActions_WithAISystemPluginFallback_UsesLegacyFormat()
        {
            // Arrange - Test backwards compatibility with AISystemPlugin
            var auditRecord = new CopilotAuditLogContent
            {
                ParsedAuditEvent = new CopilotAuditEvent
                {
                    ...
                    {
                        new AISystemPlugin { Id = "plugin1", Name = "BingWebSearch" },
                        new AISystemPlugin { Id = "plugin2", Name = "GraphConnector" }
                    }
                }
            };
            ...
            // Assert
            Assert.IsNotNull(json);
            var plugins = JsonConvert.DeserializeObject<List<AISystemPlugin>>(json);
            Assert.AreEqual(2, plugins.Count);
            Assert.AreEqual("BingWebSearch", plugins[0].Name);
            Assert.AreEqual("GraphConnector", plugins[1].Name);
        }
        ...
        public async Task SaveCopilotEvent_WithAgentActions_SavesCorrectly()
        {
            ...
            {
                // Skip if migration hasn't been run
                if (db.Database.SqlQuery<int?>("SELECT OBJECT_ID('dbo.copilot_event_agent_actions', 'U')").FirstOrDefault() == null)
                {
        ...
        public async Task SaveCopilotEvent_WithAIToolUsages_SavesCorrectly()
        {
            ...
            {
                // Skip if migration hasn't been run
                if (db.Database.SqlQuery<int?>("SELECT OBJECT_ID('dbo.copilot_event_ai_tool_usages', 'U')").FirstOrDefault() == null)
                {
        ...
        public async Task SaveCopilotEvent_WithFlowActions_SavesCorrectly()
        {
            ...
            {
                // Skip if migration hasn't been run
                if (db.Database.SqlQuery<int?>("SELECT OBJECT_ID('dbo.copilot_event_flow_actions', 'U')").FirstOrDefault() == null)
                {

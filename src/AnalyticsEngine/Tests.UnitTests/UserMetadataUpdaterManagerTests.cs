...
namespace Tests.UnitTests
{
    ...
    public class UserMetadataUpdaterManagerTests
    {
        ...
        public async Task UserMetadataUpdater_NewUserManagerInSameBatch_WorksCorrectly()
        {
            ...
            var userAId = Guid.NewGuid().ToString(); var veronicaId = Guid.NewGuid().ToString();
            ...
            var graphUsers = new List<GraphUser>
            {
                new GraphUser { UserPrincipalName = userAUpn, Id = userAId, AccountEnabled = true, Mail = userAUpn, ManagerInfo = new List<ManagerInfo> { new ManagerInfo { Id = veronicaId } } },
            ...
            using (var verifyDb = new AnalyticsEntitiesContext())
            {
                var allTestUsers = await verifyDb.users.Include(u => u.Manager).Where(u => u.UserPrincipalName == userAUpn || u.UserPrincipalName == veronicaUpn).ToListAsync();
                Assert.AreEqual(2, allTestUsers.Count);
                Assert.AreEqual(veronicaUpn, allTestUsers.First(u => u.UserPrincipalName == userAUpn).Manager.UserPrincipalName);
            }
        }
        ...
        public async Task UserMetadataUpdater_ManagerAadIdMismatch_NoDuplicateKeyError()
        {
            ...
            using (var finalVerifyDb = new AnalyticsEntitiesContext())
            {
                ...
                Assert.AreEqual(2, allTestUsers.Count, "Jorge should NOT be duplicated despite AAD ID mismatch");
                Assert.AreEqual(managerUpn, allTestUsers.First(u => u.UserPrincipalName == employeeUpn).Manager.UserPrincipalName);
            }
        }

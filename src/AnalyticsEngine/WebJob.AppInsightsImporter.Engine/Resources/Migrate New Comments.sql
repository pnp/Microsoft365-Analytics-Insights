...
DECLARE comments_to_update_cursor CURSOR  
    FOR 
		select originalComments.id, ${STAGING_TABLE_COMMENTS}.ParentCommentSharePointId, originalComments.url_id from page_comments originalComments
		inner join ${STAGING_TABLE_COMMENTS} on originalComments.sp_id = ${STAGING_TABLE_COMMENTS}.[sp_id]
		where 
			ISNULL(originalComments.parent_id, 0) = 0
			and ISNULL([${STAGING_TABLE_COMMENTS}].ParentCommentSharePointId, 0) > 0

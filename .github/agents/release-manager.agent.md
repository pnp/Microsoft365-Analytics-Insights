...
| `npx vitest run src/i18n` → *"does not pass English off as a translation"* | Nobody satisfied the compiler by copying the English across. | **Blocker.** This is the failure mode that produces a half-Spanish page while every other check is green. |
| `npx vitest run src/i18n` → *"keeps the same placeholders as the English text"* and *"supplies every placeholder each string needs, and none it does not"* | A `{count}` present in one language and not the other, or never supplied at the call site, renders as literal `{count}` on screen. | **Blocker.** Cheap to fix, embarrassing to ship. |
| `npx vitest run src/i18n` → *"formats every number and date through the portal language"* | No surviving `toLocaleString()`. | **Blocker.** `1,234` means one thousand two hundred and thirty-four in English and one point two three four in Spanish — a figure in the wrong locale is wrong by a factor of a thousand. |

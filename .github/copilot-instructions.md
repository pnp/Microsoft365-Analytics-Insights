...
- `npx vitest run src/i18n` additionally fails on: a user-facing string that never reached the catalog (with a file/line worklist), English pasted into the Spanish catalog to silence the compiler, a `{placeholder}` that differs between languages, and a key that does not match its module.
